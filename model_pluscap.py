...
from data_preproposs import  get_data_cap
from layers import *
...
#     def imgs_encoder(self):
#         with tf.variable_scope('imgs') as scope:
#             W1_img = tf.get_variable('W1_img', [4096, self.wd_hidden_dim*2], tf.float32, 
#                                     initializer = tf.contrib.layers.xavier_initializer(uniform=False)) 
#             b1_img = tf.get_variable('b1_img', [self.wd_hidden_dim*2], tf.float32, 
#                                     initializer = tf.contrib.layers.xavier_initializer(uniform=False)) 
#             
#             W2_img = tf.get_variable('W2_img', [self.wd_hidden_dim*2, self.wd_hidden_dim*2], tf.float32, 
#                                     initializer = tf.contrib.layers.xavier_initializer(uniform=False)) 
#             b2_img = tf.get_variable('b2_img', [self.wd_hidden_dim*2], tf.float32, 
#                                     initializer = tf.contrib.layers.xavier_initializer(uniform=False)) 
#             
#             imgs_low = tf.tanh(tf.matmul(self.imgs,W1_img)+b1_img)     
#             self.imgs_vec = tf.tanh(tf.matmul(imgs_low, W2_img) + b2_img)
...
#                                      tf.contrib.layers.xavier_initializer(uniform=False))
            ...
            b_mid = tf.get_variable('b_mid', [self.st_hidden_dim*2], tf.float32, initializer = tf.contrib.layers.xavier_initializer(uniform=False))
...
#                 context = update_context(context, self.img_seq[i] ,simi, t, W_mid, b_mid)     # 更新插入图片后的上下文
    ...
    def get_feed_dict(self, text_dir, captions_dir, img_dir, filename, flag):
        text, caps, imgs, la = get_data_cap(text_dir, captions_dir, img_dir, filename, flag)
        if caps == False:
            return False
        fd = {
            self.text: text,
